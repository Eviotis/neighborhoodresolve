...
export default function AdminPage() {
  ...
  async function grantLife(userId: string) {
    ...
    if (!profile) return
    await supabase.from('profiles').update({ lives_remaining: (profile.lives_remaining || 1) + 1 }).eq('id', userId)
    loadData()
  }
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        ...
        {flagged.length > 0 && (
          <div className="bg-red-50 border border-red-100 rounded-2xl px-4 py-3 mb-4">
            <p className="text-xs font-medium text-red-700 mb-1">Possible abuse detected</p>
            {flagged.map(p => (
        ...
        <div className="flex gap-0 border-b border-gray-100 mb-4">
          {['cases', 'residents'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 text-xs font-medium capitalize border-b-2 transition-colors ${activeTab === tab ? 'border-green-500 text-green-700' : 'border-transparent text-gray-400'}`}>
              {tab} {tab === 'residents' ? `(${profiles.length})` : `(${cases.length})`}
            </button>
          ))}
        </div>
        ...
        {loading ? (
              ...
              <div key={c.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-2 items-center">
                    <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-lg">{c.priority_score || 5}%</span>
          ...
          <div className="space-y-2">
            {profiles.length === 0 ? (
              <p className="text-center text-sm text-gray-400 py-8">No residents found</p>
            ) : profiles.map(p => (
              <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-800">{p.email || 'No email'}</p>
                    <p className="text-xs text-gray-400">
                      {p.is_anonymous ? `Neighbor${p.neighbor_number} · ` : ''}{p.community_code} · Level {p.access_level} · {p.report_count || 0} reports · Lives: {p.lives_remaining || 0}
                    </p>
                  </div>
                  <div className="flex gap-1 flex-col items-end">
                    ...
                    <button onClick={() => grantLife(p.id)} className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded-lg">+ Grant Life</button>
                  </div>
                </div>
              </div>
            ))}
